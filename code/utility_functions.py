...
def train_test_split(table, trainfrac=0.80):
    ...
    print(f'num train={len(train.index):5d}, num test={len(test.index):5d}')
    return train, test
...
def calculate_ref_limit(entropies, z=2.58):
    ...
    print(f'Native avg={avg:.3f}, stdev={stdev:.3f}, z={z:.3f}, ref limit={ref_limit:.3f}')
    return ref_limit
...
def draw_lnprob_dist(lnprobs, order=None, file=None):
    ...
    n, bins, patches = plt.hist(x=lnprobs, bins='auto', color='#0504aa', alpha=0.75, rwidth=0.85)
    plt.grid(axis='y', alpha=0.75)
    plt.xlabel('Log2 probability'+(" -- Markov order "+str(order) if order else ""))
    plt.ylabel('Frequency of form')
    plt.title('Log2 probability '+r'$(n='+cnt+', \mu='+avg+', \sigma='+std+')$')
    maxfreq = n.max()
...
def draw_dist(x, title='Distribution of Statistic'):
    ...
    n, bins, patches = plt.hist(x=x, bins='auto', color='#0504aa', alpha=0.75, rwidth=0.85)
    plt.grid(axis='y', alpha=0.75)
    plt.xlabel('Statistic')
    plt.ylabel('Frequency')
    plt.title(title+r' $(n='+cnt+', \mu='+avg+', \sigma='+std+')$')
    maxfreq = n.max()
    # Set a clean upper y-axis limit.
    plt.ylim(ymax=np.ceil(maxfreq / 10) * 10 if maxfreq % 10 else maxfreq + 10)
    plt.show()
