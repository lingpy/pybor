...
class Markov():
    ...
    def train(self, normalize='kneserney', smoothing=None):
        ...
        if normalize=='kneserney':
            self._lm = lm.KneserNeyInterpolated(order, vocabulary=vocab, discount=smoothing)
    ...
    def entropies(self, base=2):
        ...
        self.entropies = self.calculate_entropies(self.now['sequences'], base=base)
    ...
    def calculate_entropies(self, sequences, base=2):
        ...
        tokens = [list(pp.pad_both_ends(pp.pad_both_ends(
            t, pad_right=False, n=order), pad_left=False, n=2)) for t in tokens]
