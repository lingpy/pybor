...
class DualMarkov(Markov):
    ...
    def __attrs_post_init__(self):
        ...
        self.native = MarkovWord(
            tokens=nativetokens, data=self.data, model=self.model, order=self.order,
            smoothing=self.smoothing)

        loantokens = [token for _, token, status in self.data if status == 1]
        self.loan = MarkovWord(
            tokens=loantokens, data=self.data, model=self.model, order=self.order,
            smoothing=self.smoothing)
