...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul  3 12:49:36 2020
...
Run test file:
$ pytest tests/test_wold.py -—cov=pybor.data

Or to save as html report:
$ pytest tests/test_wold.py -—cov=pybor.wold --cov-report=html

Or to report line numbers of missing coverage:
$ pytest tests/test_wold.py --cov=pybor.wold --cov-report term-missing

Or to perform a specific test:
$ pytest tests/test_wold.py::test_gettable --cov=pybor.wold --cov-report term-missing
...
def test_language_table_gen():
    gen = wold.language_table_gen("English")
    ...
    gen = wold.language_table_gen(["English", "Hup"])
    language, table = next(gen)
    assert language == "English"
    assert len(table) == 1516  # As of June 18, 2020
    # assert len(table[0]) == 3
...
def test_language_table_gen_donor():
    gen = wold.language_table_gen("English", donor_num=1)
    language, table = next(gen)
    assert language == "English"
...
def test_access_functions():
    wolddb = wold.get_wold_access()
    language = "Hup"
    languages = wold.check_wold_languages(wolddb, language)
    print(languages)
...
def test_get_donor_table():
    wolddb = wold.get_wold_access()
    table = wold.get_native_donor_table(
        wolddb, "English", form="Tokens", classification="Borrowed", donor_num=0
    )
    assert len(table) == 1516
    # assert len(table[0]) == 3

    table = wold.get_native_donor_table(
        wolddb, "English", form="Tokens", classification="Borrowed", donor_num=1
    )
...
def test_apply_function():
    ...
    def get_user_fn(detect_type, model_type):
        def fn_example(
            language, form, table, detect_type=detect_type, model_type=model_type
        ):
    ...
    wold.apply_function_by_language(
        languages=["English", "Hup"], form="FormChars", function=fn
    )

    donor_num = 1
    fn = get_user_fn("dual", "recurrent")
    wold.apply_function_by_language(
        languages=["English", "Hup"], form="FormChars", function=fn, donor_num=donor_num
    )
