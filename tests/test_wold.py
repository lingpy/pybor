...
def test_get_table():
    ...
    assert wolddb
    table = wolddb.get_table(language='English', form='FormChars', classification='Borrowed')
    assert table
...
def test_get_donor_table():
    wolddb = wold.get_wold_access()
    table = wold.get_native_donor_table(wolddb, 'English', form='Tokens',
...
def test_apply_function():
    ...
    wold.apply_function_by_language(languages=['English', 'Hup'], form='FormChars', function=fn)

    donor_num = 1
    fn = get_user_fn('dual', 'recurrent')
    wold.apply_function_by_language(languages=['English', 'Hup'],
