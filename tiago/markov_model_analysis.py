...
def calculate_test_statistic_between_distributions(x, y, test="ks"):
    ...
    if test == "t":
        ...
        statistic = MeanDif(
            dif=statistics.mean(x) - statistics.mean(y),
            x=statistics.mean(x),
            y=statistics.mean(y),
        )
    else:
...
def calculate_randomization_test_between_distributions(
    values, selector, test="ks", n=1000
):
...
def analyze_word_distributions(
    ...
    calculate_randomization_test_between_distributions(
        entropies, selector=selector, test=test, n=n
    )

    return mlm


# TODO: move to top, or separate
def read_table(language):
    tabledir = "tables/"

    with open(tabledir + language + ".tsv") as tablefile:
        # TODO: add some metadata to emulate pandas
        table = [
            {
                "concept": row["concept"],
                "form": row["form"],
                "formchars": row["formchars"],
                "segments": row["segments"],
                "scas": row["scas"],
                "loan": row["loan"] == "True",
                "borrowedscore": float(row["borrowedscore"]),
            }
            for row in csv.DictReader(tablefile, delimiter="\t")
        ]

    return table


def analyze_language_word_distributions(
    language=None, form="formchars", test="ks", n=1000, logebase=True
):
...
def analyze_word_distributions_native_basis(
    ...
    graph_word_distribution_entropies(
        ...
        + " native and loan entropy distribution - undifferentiated fit",
        graphlimit=7,
        figurequal="-native-basis-native-loan-entropies",
    )
...
def analyze_language_tokens_test(
    tokens=None, selector=None, model="KNI", order=3, smoothing=0.5, test="ks"
):
...
def differentiated_randomization_test(
    tokens, selector, n=100, order=3, model="KNI", smoothing=0.5, test="t"
):
    # ts: t, ks, md
...
def analyze_language_word_distributions_native_basis(
    language=None, form="formchars", test="ks", n=1000, logebase=True
):
    print(f"Language={language}, form={form}")
    ...
    differentiated_randomization_test(
        tokens=tokens,
        selector=selector,
        n=n,
        order=3,
...
def fit_native_loan_models(
    tokens=None, ground=None, model="KNI", order=3, smoothing=0.5
):
    if ground is None:
    ...
    nativemodel = MarkovCharLM(
        tokens_native, model=model, order=order, smoothing=smoothing
    )
    nativeentropies = nativemodel.analyze_tokens(tokens)

    tokens_loan = [
        token for token, select in zip(tokens, ground) if select == False
    ]
    loanmodel = MarkovCharLM(
        tokens_loan, model=model, order=order, smoothing=smoothing
    )
...
def evaluate_models_for_test(
    nativemodel=None, loanmodel=None, val_tokens=None, valground=None
):
...
def analyze_native_loan_dual_basis(
    train=None, val=None, form="formchars", smoothing=0.5
):
    trainground = [entry["borrowedscore"] < 0.375 for entry in train]
    valground = [entry["borrowedscore"] < 0.375 for entry in val]

    train_tokens = [entry[form] for entry in train]
    val_tokens = [entry[form] for entry in val]

    nativemodel, loanmodel = fit_native_loan_models(
        tokens=train_tokens, ground=trainground, smoothing=smoothing
    )
    metrics = evaluate_models_for_test(
        nativemodel, loanmodel, val_tokens, valground
    )
    return metrics


def analyze_native_loan_dual_basis2(
    train=None, val=None, form="formchars", smoothing=0.5
):
    trainground = train.borrowedscore < 0.375  # native
    valground = val.borrowedscore < 0.375  # native
    nativemodel, loanmodel = fit_native_loan_models(
        tokens=train[form], ground=trainground, smoothing=smoothing
    )
    metrics = evaluate_models_for_test(
        nativemodel, loanmodel, val[form], valground
    )
    return metrics


def language_word_discrimination_dual_basis(
    language=None, form="formchars", smoothing=0.5, trainfrac=0.8
):
    print()
    print(f"Language={language}, form={form}")
    tabledir = "tables/"
...
def fit_native_model(
    tokens=None, ground=None, model="KNI", order=3, smoothing=0.5, p=0.995
):
    if ground is None:
        return None  # Must have ground to fit.
...
def analyze_native_loan_native_basis(
    train=None, val=None, form="formchars", smoothing=0.5, p=0.995
):
    ...
    nativemodel, ref_limit = fit_native_model(
        train_tokens, trainground, smoothing=smoothing, p=p
    )
    # Evaluate on test set.
    valentropies = nativemodel.analyze_tokens(val_tokens)
...
def analyze_native_loan_native_basis2(
    train=None, val=None, form="formchars", smoothing=0.5, p=0.995
):
    # table with forms and ground.
    # Ground variable for native words of train and test sets.
    ...
    nativemodel, ref_limit = fit_native_model(
        train[form], trainground, smoothing=smoothing, p=p
    )
    # Evaluate on test set.
...
def language_word_discrimination_native_basis(
    language=None, form="formchars", smoothing=0.5, trainfrac=0.8, p=0.995
):
    print()
    print(f"Language={language}, form={form}")
    table = read_table(language)
    ...
    train, val = train_test_split(table, trainfrac=trainfrac)
    analyze_native_loan_native_basis(
        train, val, form=form, smoothing=smoothing, p=p
    )
...
# Overall script for individual language test - native word basis
def k_fold_language_word_discrimination_native_basis(
    language, form="formchars", smoothing=0.5, k_fold=10, p=0.995
):
    ...
    my_table = read_table(language)
    kf = KFold(n_splits=k_fold, shuffle=True)
    predictions = []
    for train_index, val_index in kf.split(my_table):
        train, val = [], []
        for idx, entry in enumerate(my_table):
            if idx in train_index:
                train.append(entry)
            elif idx in val_index:
                val.append(entry)

        results = analyze_native_loan_native_basis(
            train, val, form=form, smoothing=smoothing, p=p
        )
...
def k_fold_language_word_discrimination_dual_basis(
    language, form="formchars", smoothing=0.5, k_fold=10
):
    # language = language archive name, e.g., English
    print()
    print(f"Language={language}, form={form}")
    ...
    for train_index, val_index in kf.split(my_table):
        ...
        results = analyze_native_loan_dual_basis(
            train, val, form=form, smoothing=smoothing
        )
        predictions.append(results._asdict())
...
def entropy_train_val(
    train_tokens=None, val_tokens=None, order=3, model="KNI", smoothing=0.5
):
...
def k_fold_entropy(segments, k_fold=5, order=3, model="KNI", smoothing=0.5):
    ...
    return [
        [sz, k_fold, math.ceil(sz * frac), model, order, smoothing],
        summary,
    ]
...
def k_fold_entropy_for_language(
    language=None,
    form="formchars",
    selector=None,
    k_fold=5,
    ...
    print()
    print(f"Language={language}, form={form} - {k_fold}-fold entropy")
    tabledir = "tables/"
    table = read_table(language)
    ...
    if selector is None:
        ...
        segments = [entry[form] for entry in table]
    else:
        # print(f'Selector={inspect.getsource(selector)}')
        print("Subset selected: See sample and val sizes.")

        segments = [entry[form] for entry in table if selector(entry)]

    # Handle scalar or list for smoothing.
    smoothing = (
        np.asarray([smoothing])
        if np.isscalar(smoothing)
        else np.asarray(smoothing)
    )
    for sm in smoothing:
        summary = k_fold_entropy(
            segments, k_fold=k_fold, order=order, model=model, smoothing=sm
        )
